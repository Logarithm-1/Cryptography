    ...
    public override func encrypt(_ plaintext: String) -> String {
        ...
        for char in plaintext.uppercased() {
            ...
            if(alphabet.contains(encryptedChar)) {
                ...
                //Go back through the rotors. Since we are going through backwards we want to use decrypt instead of encrypt.
                for rotor in rotors {
                    if let cipher = key[rotor] {
                ...
                //If valid char, add to encrypted message + rotate rotors
                if(validChar) {
